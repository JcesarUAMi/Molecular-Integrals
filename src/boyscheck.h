...
template<typename T> Boys;

template<typename T> T Boys<T>::boysUpward(T x, int n) {

  T result, value;
  value = 1.0/(2.0*x);
  result = SplinesBoys(9, x);
  for (int i = 50; i < n; i++) {
    result *= value * (2*i + 1);
    result -= value * exp(-x);
  }
...
template<typename T> T Boys<T>::boysDownward(T integral, T x, int m, int p) {
